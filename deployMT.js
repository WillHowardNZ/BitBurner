...
export async function main(ns) {
  ...
  Forked from deloySH.js, this version caps the maximum number of threads at 200, and doesn't kill existing processes.*/
	...
	const threadMax = 200; /* Prevent scritps being too many threads */
	...
	/* ns.killall(host); */
	var threadsCalc = Math.floor((ns.getServerMaxRam(host) - ns.getServerUsedRam(host)) / ns.getScriptRam(script));
	if (threadMax < threadsCalc) {
		threads = threadMax;
	} else threads = threadsCalc;
